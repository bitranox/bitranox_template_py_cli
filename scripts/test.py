...
def _run_pytest_step(
    ...
    if enable_coverage:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            pytest_result = run_fn(
                [
                    ...
                    "pytest",
                    "-m",
                    "not slow",  # Exclude slow integration tests from normal runs
        ...
        pytest_result = run_fn(
            ["python", "-m", "pytest", "-m", "not slow", config.pytest_verbosity],
            capture=False,
            label="pytest-no-cov",
        )
        if pytest_result.code != 0:
...
def run_slow_tests(*, verbose: bool = False) -> None:
    """Run only tests marked as slow (integration tests).

    These tests require external resources (SMTP server, etc.) and are
    excluded from normal CI runs. Use this target to run integration tests
    locally when the required environment is configured.
    ...
    if result.code != 0:
        click.echo("[test-slow] Some tests failed", err=True)
        raise SystemExit(result.code)

    click.echo("[test-slow] Slow tests completed successfully")
